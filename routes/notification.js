	...
	pool.query(sql,[date,user],function(err,apply){
      ...
      pool.query(sql2,[date,user],function(err,forms){
        if(err){
          ...
          var sql3="SELECT u.user_name,u.user_pic,n.need_name,f.form_id,(SELECT GROUP_CONCAT(date) FROM need_schedule where form_id=f.form_id) as schedule FROM user u INNER JOIN needs n ON u.user_id=n.user_id INNER JOIN forms f ON n.need_id=f.need_id where f.last_updated>? and flag=1 and f.user_id=?";
